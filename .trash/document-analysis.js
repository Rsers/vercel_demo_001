...
export default async function handler(req, res) {
  ...
  try {
    ...
    const systemPrompts = {
      ...
      ocr: '你是一个专业的OCR文字识别专家，能够从图片中提取文字内容。'
    };
    ...
    if (!response.ok) {
      ...
      return res.status(400).json({
        ...
        details: '这展示了为什么需要服务端 API - 文档分析需要 AI 服务支持'
      });
    }
    ...
    const processedData = {
      ...
      apiInfo: {
        ...
        note: '这是通过 Vercel API 集成的 DeepSeek 文档分析服务'
      }
    };
    ...
    console.error('❌ 文档分析 API 错误:', error);
    res.status(500).json({
      error: '文档分析服务调用失败',
      message: error.message,
      details: '这展示了为什么需要服务端 API - 文档分析服务调用可能失败，需要统一的错误处理'
    });
  }
}
